...
{
    @Modifying
    @Query(value = "UPDATE bill SET state = :billState,verification_code =:verificationCode WHERE id = :billId", nativeQuery = true)
    @Transactional
    void changeBillStatus(@Param("billState") BillState billState,@Param("verificationCode") String verificationCode, @Param("billId") Long billId);

    @Query(value = "SELECT * FROM bill WHERE state = :billState AND client_id = :clientId ", nativeQuery = true)
    Optional<List<Bill>> findPaidBills(@Param("billState") BillState billState, @Param("clientId") Long clientId);

    @Query(value="SELECT * FROM bill WHERE state=:billState and clientId = :clientId",nativeQuery = true)
